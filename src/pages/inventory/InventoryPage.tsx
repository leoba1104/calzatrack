...
export function InventoryPage() {
  ...
  const { data: productos, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (search) {
        query = query.or(`nombre.ilike.%${search}%,codigo.ilike.%${search}%,marca.ilike.%${search}%`)
      }
                ...
                productos?.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50 transition-colors cursor-pointer">
                    <td className="px-4 py-3">
                      ...
                      {p.categoria && <p className="text-xs text-gray-400">{p.categoria.nombre}</p>}
                    </td>
                    <td className="px-4 py-3 text-gray-600 font-mono text-xs">{p.codigo}</td>
                    ...
                    <td className="px-4 py-3 text-right font-medium text-gray-900">{formatCRC(p.precio_venta)}</td>

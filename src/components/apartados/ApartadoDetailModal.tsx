...
export function ApartadoDetailModal({ venta, isOpen, onClose, onCompleted }: ApartadoDetailModalProps) {
  ...
  const cliente = venta.cliente as { nombre: string; apellido: string | null } | null

  const totalAbonado  = pagos.reduce((s, p) => s + p.monto, 0)
  const saldo         = venta.total - totalAbonado
  const porcentaje    = Math.min(100, Math.round((totalAbonado / venta.total) * 100))
  const { limite, dias, vencido } = diasInfo(venta.created_at)
      ...
      <div className="flex flex-col max-h-[85vh]">
        ...
        <div className="px-6 pt-5 pb-4 border-b border-gray-100 grid grid-cols-2 gap-x-8 gap-y-3">
          ...
          <div className="col-span-2">
            <div className="flex justify-between text-xs text-gray-500 mb-1.5">
              ...
              <span>Saldo: <strong className={cn(pagado ? 'text-green-700' : 'text-gray-800')}>{pagado ? '¡Pagado!' : formatCRC(saldo)}</strong></span>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-6 py-4">
            ...
            {pagos.length === 0 ? (
              <p className="text-sm text-gray-400 italic">Sin abonos registrados</p>
            ) : (
                  ...
                  <div key={p.id ?? i} className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-xl">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {metodoPagoLabel[p.tipo_pago as MetodoPago]} · {formatDate(p.fecha)}
                        {p.notas && ` · ${p.notas}`}
                      </p>
                    </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-between items-center bg-gray-50/50 shrink-0">
          {pagado ? (
            ...
            <p className="text-xs text-gray-400">Saldo pendiente: <strong className="text-gray-700">{formatCRC(saldo)}</strong></p>
          )}
